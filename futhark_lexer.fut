...
module mk_lexer (L: lexer_context) = {
  ...
  def lex_with_dead_old [n] (str: [n]u8) : [](u32, terminal) =
    let endos = traverse str
    in tabulate n (\i -> (i == n - 1 || is_produce endos[i + 1], endos[i], u32.i64 i))
       |> map (\(b, endo, end) ->
                 (b, end, to_terminal endo))
       |> filter (.0)
       |> map (\(_, e, e') -> (e, e'))

  def lex_old [n] (str: [n]u8) : opt ([](u32, terminal)) =
    let result = lex_with_dead_old str
    let is_valid =
      length result == 0
  ...
  def lex_with_dead [n] (str: [n]u8) : [](u32, terminal) =
    let endos = traverse str
    in tabulate n (\i -> (i == n - 1 || is_produce endos[i + 1], endos[i], u32.i64 i))
       |> map (\(b, endo, end) ->
                 (b, end, to_terminal endo))
