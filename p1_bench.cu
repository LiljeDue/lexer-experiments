...
// P1 prefetch pipeline: overlaps next-tile global loads with current-tile
// decoupled lookback. Uses manual warp-striped load (LoadDirectWarpStriped)
// so raw bytes are in registers during the lookback spin, then exchanges
// to block-blocked layout via shmem after the prefix is resolved.
//
// Pipeline per tile:
//   1. Exchange prev tile's warp-striped bytes → block-blocked (shmem)
//   2. Map bytes → states via shmem_to_state
//   3. Issue LoadDirectWarpStriped for NEXT tile (bytes into regs, no shmem)
//   4. Run scan + decoupled lookback (next-tile loads hide in background)
//   5. Store current tile output
//   6. Repeat with next tile
template<uint32_t BLOCK_SIZE, uint32_t ITEMS_PER_THREAD>
__global__ LB_P1
void p1_prefetch(
    state_t* __restrict__ d_compose_glb,
    ...
    uint32_t num_tiles)
{
    using BlockExchangeT = cub::BlockExchange<uint8_t, BLOCK_SIZE, ITEMS_PER_THREAD>;
    ...
    const uint32_t TILE_SIZE = BLOCK_SIZE * ITEMS_PER_THREAD;
    ComposeOp compose_op{shmem_compose};

    // Prefetch tile 0 raw bytes into registers (warp-striped, coalesced).
    uint8_t raw[ITEMS_PER_THREAD];
    {
        uint32_t glb_offs = (uint32_t)blockIdx.x * TILE_SIZE;
        cub::LoadDirectWarpStriped(threadIdx.x, d_in + glb_offs, raw,
                                   (int)min((uint64_t)TILE_SIZE, (uint64_t)size - glb_offs),
                                   (uint8_t)0);
    }

    uint32_t tile_idx = blockIdx.x;
    uint32_t glb_offs = tile_idx * TILE_SIZE;

    // Exchange warp-striped raw bytes → block-blocked via shmem.
    BlockExchangeT(temp.exchange).WarpStripedToBlocked(raw, raw);
    __syncthreads();
    ...
    for (uint32_t i = 0; i < ITEMS_PER_THREAD; i++)
        st[i] = shmem_to_state[raw[i]];

    // Prefetch next tile raw bytes while this tile's lookback runs.
    uint8_t raw_next[ITEMS_PER_THREAD];
    {
        uint32_t next_offs = glb_offs + TILE_SIZE;
        if (next_offs < size)
            cub::LoadDirectWarpStriped(threadIdx.x, d_in + next_offs, raw_next,
                                       (int)min((uint64_t)TILE_SIZE, (uint64_t)size - next_offs),
                                       (uint8_t)0);
    }

    // Scan + decoupled lookback (next-tile loads are in flight).
    if (tile_idx == 0) {
        state_t agg;
        BlockScanT(temp.scan_storage.scan).InclusiveScan(st, st, compose_op, agg);
        if (threadIdx.x == 0) tile_state.SetInclusive(0, agg);
    } else {
        PrefixOp pfx(tile_state, temp.scan_storage.prefix, compose_op, (int)tile_idx, IDENTITY);
        BlockScanT(temp.scan_storage.scan).InclusiveScan(st, st, compose_op, pfx);
    }
    __syncthreads();

    // Store current tile.
    uint32_t valid = (uint32_t)min((uint64_t)TILE_SIZE, (uint64_t)size - glb_offs);
    #pragma unroll
    for (uint32_t i = 0; i < ITEMS_PER_THREAD; i++) {
        uint32_t lid = threadIdx.x * ITEMS_PER_THREAD + i;
        if (lid < valid)
            d_states_out[glb_offs + lid] = st[i];
    }

    // Process remaining tiles using the prefetched bytes.
    tile_idx++;
    while (tile_idx < num_tiles) {
        glb_offs = tile_idx * TILE_SIZE;

        // raw_next holds warp-striped bytes for this tile; exchange to block-blocked.
        __syncthreads();
        BlockExchangeT(temp.exchange).WarpStripedToBlocked(raw_next, raw_next);
        __syncthreads();

        #pragma unroll
        for (uint32_t i = 0; i < ITEMS_PER_THREAD; i++)
            st[i] = shmem_to_state[raw_next[i]];
        ...
        // Store.
        valid = (uint32_t)min((uint64_t)TILE_SIZE, (uint64_t)size - glb_offs);
        #pragma unroll
        for (uint32_t i = 0; i < ITEMS_PER_THREAD; i++) {
            uint32_t lid = threadIdx.x * ITEMS_PER_THREAD + i;
            if (lid < valid)
...
int main(int argc, char** argv) {
    ...
    // ------------------------------------------------------------------
    // P1 prefetch: persistent kernel, overlaps next-tile loads with
    // current-tile decoupled lookback.
    // ------------------------------------------------------------------
    {
        auto kernel    = p1_prefetch<BLOCK_SIZE, ITEMS_PER_THREAD>;
        size_t p1_bytes = (size_t)size * sizeof(uint8_t) + (size_t)size * sizeof(state_t);

        printf("%-38s \n  %-36s ", "2Pass P1 BS256/IPT22 (prefetch):", "P1:");
        for (uint32_t i = 0; i < WARMUP_RUNS; i++) {
            reset(ts, nlb);
            kernel<<<nlb, BLOCK_SIZE>>>(
                d_compose_glb, d_to_state_glb, d_in, d_states_out, ts, size, nlb);
