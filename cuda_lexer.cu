...
template<typename I, I BLOCK_SIZE, I ITEMS_PER_THREAD>
__global__ void
lexerTwoKernels1WorseCopy(LexerCtx ctx,
                uint8_t* d_in,
                state_t* d_states_out,
                volatile State<state_t>* state_states,
                I size,
...
template<typename I, I BLOCK_SIZE, I ITEMS_PER_THREAD>
__global__ void
lexerTwoKernels2WorseCopy(state_t* d_states_in,
                 I* d_index_out,
                 token_t* d_token_out,
                 volatile State<I>* index_states,
                 I size,
...
void testLexerTwoKernelsWorseCopy(uint8_t* input,
               size_t input_size,
               uint32_t* expected_indices,
               token_t* expected_tokens,
               size_t expected_size) {
    ...
    for (I i = 0; i < WARMUP_RUNS; ++i) {
        lexerTwoKernels1WorseCopy<I, BLOCK_SIZE, ITEMS_PER_THREAD><<<NUM_LOGICAL_BLOCKS, BLOCK_SIZE>>>(
            ctx,
            d_in,
            d_states_out,
            d_state_states,
        ...
        cudaDeviceSynchronize();
        cudaMemset(d_dyn_index_ptr, 0, sizeof(uint32_t));
        lexerTwoKernels2WorseCopy<I, BLOCK_SIZE, ITEMS_PER_THREAD><<<NUM_LOGICAL_BLOCKS, BLOCK_SIZE>>>(
            d_states_out,
            d_index_out,
            d_token_out,
            d_index_states,
...
int main(int32_t argc, char *argv[]) {
    ...
    printf(PAD, "Lexer Two Kernels Worse Copy:");
    testLexerTwoKernelsWorseCopy(input, input_size, expected_indices, expected_tokens, expected_indices_size);
